<template>
  <div class="page-body card">
    ...
    <div class="card-block" v-if="!loading && !messages.length">
      <div class="col-md-12">
        <div class="alert alert-info">
          <i class="fa fa-exclamation-circle fa-2x"></i> Trash is Empty
        </div>
      </div>
    </div>
    <data-table
    ...
    <div class="card-footer" v-if="!loading && messages.length">
      <div class="pull-right">
        <submit-button
          title="Empty Trash"
          v-on:submit="emptyTrash"
          styling="btn btn-inverse btn-round waves-effect waves-light btn-sm"
        />
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    loadData(itemsPerPage, pageNumber) {
      ...
      this.$http
        .get(
          'messages/getMessages?messageStatus=4&userCode=' + this.user.username
        )
        .then(result => {
          this.loading = false;
    ...
    buttonEvent(item, action) {
      switch (action) {
        case 'delete':
          ...
          this.$http
            .post('messages/deleteMessage', message)
            .then(result => {
              this.loading = false;
              if (result.data.success) {
                this.loadData(10, 1);
