...
export default {
  ...
  methods: {
    ...
    loadData(itemsPerPage, pageNumber) {
      this.loading = true;
      this.$http
        .get('imprestSurrender/getAllSurrenderRequests')
        .then(result => {
          if (result.data.success) {
            this.loading = false;
            result.data.data.pendingRequests.forEach(element => {
            ...
            this.surrenderReqs = result.data.data.pendingRequests;
          } else {
